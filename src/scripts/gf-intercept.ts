...
export default function initGFIntercept(opts: InitOptions) {
  ...
  const forms = Array.from(
    root.querySelectorAll<HTMLFormElement>(
      '.gf-wrap form[id^="gform_"], form[id^="gform_"]'
    )
  );
  ...
  for (const form of forms) {
    ...
    function clearValidation() {
      ...
      form
        .querySelectorAll('.gfield_error')
        .forEach((el) => el.classList.remove('gfield_error'));

      form
        .querySelectorAll('.gfield_validation_message')
    ...
    function ensureFieldMessageEl(fieldWrap: Element) {
      let msg = fieldWrap.querySelector(
    ...
    function setFieldError(inputId: string, message: string) {
      const baseId = String(inputId).split('.')[0];

      const field = form.querySelector(
        `#field_${formId}_${baseId}`
      );
      ...
      const input = field.querySelector(
        `[name="input_${inputId}"], [name="input_${baseId}"]`
      ) as HTMLElement | null;
    ...
    async function handleSubmit(e: Event) {
      ...
      try {
        const res = await fetch(
          `${wpBase}/wp-json/astro/v1/gf/submit`,
          {
            method: 'POST',
            headers: {
