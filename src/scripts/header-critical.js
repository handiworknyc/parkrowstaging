...
// ---------------------------------------------------------
// 1. Expose SAL globally
// ---------------------------------------------------------
window.sal = sal;


// ---------------------------------------------------------
// 2. Stable SAL initialization (race-condition-proof)
// ---------------------------------------------------------
function initSal() {
  if (typeof window.sal !== "function") return;
  if (typeof HW === "undefined") return;

  // Prevent double-initialization
  if (initSal._ran) return;
  initSal._ran = true;
  ...
  if ($$(".rowindex-1").length > 0) {
    HW.requestTimeout(() => {
      const row = $$(".rowindex-1")[0];
      if (row) row.classList.add("show-me");
    }, delay);
  }
}
...
// ---------------------------------------------------------
// 3. Safe attach: handle all timing scenarios
// ---------------------------------------------------------

// Try immediately (in case DOM already loaded)
queueMicrotask(initSal);

// If DOM isn't loaded yet
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initSal, { once: true });
} else {
  // If DOM is already loaded but earlier code ran before SAL was set
  setTimeout(initSal, 0);
}
