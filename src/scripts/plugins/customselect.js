...
HW.multiselectClick = new CustomEvent('multiselectClick');
...
function MultiselectDropdown(options){
  ...
  document.querySelectorAll("select[multiple]:not(.has-multi)").forEach((el,k)=>{
	el.querySelectorAll('option').forEach(option => {
	    option.selected = false;
	});

	var myph = el.options[0].textContent;
    ...
    var optextwrap = newEl('div', { class: 'optext-wrap' });
		optextwrap.classList.add('pr', 'ofh');
    ...
    el.loadOptions=()=>{
	  ...
	  var myoptextwrap = div.querySelectorAll('.optext-wrap')[0];
	  
      Array.from(el.options).map(o=>{
        var op=newEl('div',{class:o.selected?'checked':'',optEl:o})
        var ic=newEl('input',{type:'checkbox',checked:o.selected});

        var icspan=newEl('span',{type:'checkbox'});
        var opinner=newEl('div',{class:'op-inner'});

        opinner.classList.add('pr');
        
        opinner.appendChild(ic);
        opinner.appendChild(icspan);

        opinner.appendChild(newEl('label',{text:o.text}));
		op.appendChild(opinner);
      ...
      div.refresh=()=>{
        div.querySelectorAll('span.placeholder').forEach(t=>div.removeChild(t));
        div.querySelectorAll('span.optext').forEach(t=>myoptextwrap.removeChild(t));

        var sels=Array.from(el.selectedOptions);
        if(sels.length>(el.attributes['multiselect-max-items']?.value??5)){
          myoptextwrap.appendChild(newEl('span',{class:['optext','maxselected'],text:sels.length+' '+config.txtSelected}));          
        }
        else{
          sels.map(x=>{
            ...
            myoptextwrap.appendChild(c);
          });
        }
        
		
        if(0==el.selectedOptions.length) div.appendChild(newEl('span',{class:'placeholder',text:myph}));

            // Check if sum of optext div widths exceeds the width of optextwrap
            let optexts = myoptextwrap.querySelectorAll('.optext');
            let totalWidth = 0;
            optexts.forEach(optext => {
                totalWidth += optext.offsetWidth;
            });

            if (totalWidth > myoptextwrap.offsetWidth) {
                myoptextwrap.classList.add('optext-of');
            } else {
                myoptextwrap.classList.remove('optext-of');
            }
      };
      div.refresh();
    }
    ...
    div.addEventListener('click',()=>{
      ...
      var $myrow = div.closest('.mktoFormRow');
      console.log($myrow);
      if($myrow) {
	      $myrow.classList.add('selected');
      }
      window.dispatchEvent(HW.multiselectClick);
    });
